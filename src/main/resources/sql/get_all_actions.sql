...
SELECT *, (extract(epoch FROM (action_time)::timestamp without time zone))-(extract(epoch FROM date_trunc('day', (action_time)::timestamp without time zone))) AS seconds_from_midnight FROM (
